...
return array(
	...
	/*
	 * Automatic isolation of NEW tables.
	 *
	 * When true, any table created at runtime (via CREATE TABLE) that is not a
	 * known core OpenCart table is treated as belonging to the extension that
	 * created it: a `tenant_id` column is injected into its definition and the
	 * table is recorded in `<prefix>tenant_scoped_table` so it stays scoped on
	 * every future request. This means installing ANY extension isolates its
	 * data per tenant with no manual configuration or code changes.
	 */
	'auto_isolate_new_tables' => true,

	/*
	 * Opt-out: extension tables that must remain SHARED across tenants (rare,
	 * e.g. a globally shared lookup an extension intentionally creates). Listed
	 * without prefix. These are never auto-isolated.
	 */
	'global_extension_tables' => array(),

	/*
	 * Baseline of core OpenCart tables (without prefix). Used to tell whether a
	 * freshly created table is "new" (belongs to an extension) and therefore a
	 * candidate for automatic isolation. Do not edit unless the core schema
	 * changes.
	 */
	'core_tables' => array(
		'address',
